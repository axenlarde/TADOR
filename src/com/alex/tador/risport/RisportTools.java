package com.alex.tador.risport;
...
public class RisportTools
	 ...
	 */
	public static ArrayList<BasicPhone> doPhoneSurvey(CUCM cucm, ArrayList<BasicPhone> phoneList)
		{
		try
			{
			...
			while(index < phoneList.size())
				...
				if((temp.size()==maxPhoneRequest) || (index == phoneList.size()))
					...
					SelectCmDeviceReturn selectResponse = cucm.getRISConnection().selectCmDevice("",criteria);
	...
	public static ArrayList<BasicPhone> getDeviceByIP(CUCM cucm, String IP)
		{
		ArrayList<BasicPhone> result = new ArrayList<BasicPhone>();

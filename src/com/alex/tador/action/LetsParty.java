...
import com.alex.tador.utils.Variables.Mode;
...
public class LetsParty extends Thread
	...
	public void run()
		...
		try
			...
			Mode mode = Mode.valueOf(UsefulMethod.getTargetOption("mode"));
			...
			if((mode.equals(Mode.full)) || (mode.equals(Mode.survey)))
				{
			...
			if(mode.equals(Mode.full))
				...
				for(Office o : Variables.getOfficeList())
					...
					for(BasicPhone bp : o.getPhoneList())
						...
						try
							{
							AXLTools.updatePhoneLoad(bp.getName(), UsefulMethod.getFirmware(bp.getModel(),bp.getProtocol()), Variables.getSrccucm());
							}
						catch (Exception e)
							{
					...
					Variables.getLogger().debug("2nd : phone load updated for device pool : "+o.getName());
					}
			 ...
			 */
			if((mode.equals(Mode.full)) || (mode.equals(Mode.reset)))
				{
